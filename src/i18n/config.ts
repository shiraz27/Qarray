...
// Per-namespace locale files. Add new keys here and run scripts/i18n-translate.mjs
// to fill in fr + ar via Lovable AI Gateway.
const NAMESPACES = [
...
// Vite eager glob import — bundles all JSON at build time.
const files = import.meta.glob('./locales/*/*.json', { eager: true }) as Record<string, { default: Record<string, string> }>;
...
const initialLng = stored && (LANGS as readonly string[]).includes(stored) ? stored : 'en';
...
/* eslint-disable */
const _legacy = { welcomeBack: "Welcome Back",
