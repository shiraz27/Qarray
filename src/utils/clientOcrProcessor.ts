...
export type { OcrMode, OcrPsm };

export interface OcrRunOptions {
  /** Force language pack (e.g. "fra", "ara", "fra+ara"). Skips auto-detect. */
  langs?: string;
  /** Tesseract page segmentation mode (default '6'). */
  psm?: OcrPsm;
  /** Free-form context (chapter/subject/book/teacher/notes) saved with result. */
  contextHint?: string;
}
...
/**
 * Upscale a small image so Tesseract has enough pixels per glyph. No new deps.
 */
...
/**
 * Extract text from a standalone image using Tesseract.js. French-default, with
 * a quick probe pass to switch to Arabic-first or English-first when the image
 * is mostly one of those scripts.
 */
...
export async function processResourceOCR(
  ...
  try {
    ...
    // Expand split-PDF manifests to their underlying per-page URLs so OCR
    // runs page-by-page exactly like a regular multi-page PDF would.
